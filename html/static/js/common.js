...
(function($){
	gb.CommonFunction = function(){
		...
		tabMenu = function(){
			gb.tabMenu.find('a').each(function(){
				var trg = $(this);
				trg.on('click', function(){
					var	id = trg.data('tab-id'),
				...
				$('.change-tab').on('click', function(e){
					...
					var id_ = $(this).attr('href').substr(1);
					
					trg.filter('[data-tab-id='+id_+']').trigger('click');
				});
			});
		},
		...
		dropDown2 = function(){
			gb.dropDown2.each(function(){
				var trg = $(this);
				
				trg.find('dt a').on('click', function(){
					var currentItem = $(this).closest('dl');
					if(currentItem.hasClass('active')){
						currentItem
						.removeClass('active')
						.find('dd').stop().slideUp(300);
					}else {
						currentItem
						.find('dd').stop().slideDown(300, function(){
							currentItem.addClass('active');
						});
						
						if(trg.hasClass('toggle')){
							gb.dropDown2.find('dl').not(currentItem)
							.removeClass('active')
							.find('dd').stop().slideUp(300);
						}
					}
				...
				trg.find('.btn-fold').on('click', function(){
					var currentItem = $(this).closest('dl');
					currentItem
					.removeClass('active')
					.find('dd').stop().slideUp(300);
		...
		motion = function(){
			gsap.to(".bg-area", {width:0, opacity:0, duration:0.6});
			gsap.fromTo("h2", {opacity:0, y:100, scale:1.3}, {opacity:1, duration:1, delay:0.6});
			gsap.delayedCall(1, addMotion);
			...
			function addMotion(){
				...
				gsap.fromTo(".section-wrap-01 dl dt", {
					...
					onComplete:function(){
						...
						gsap.fromTo(".section-wrap-01 .btn-wrap", {opacity:0, x:50}, {opacity:1, duration:0.8, x:0});
					}
				});
		...
		modal = function(){
			$('.active-modal').on('click', function(){
				...
				$.get('/weeples/html/pop/popup.html', function(data){
	...
	if(gb.sideMenu.length){
		window.addEventListener('scroll', function(){
			if(document.documentElement.scrollTop 
				>= $('.contents-wrap').offset().top - gb.header.height()){
			...
			if(document.documentElement.scrollTop 
				...
				gb.sideMenu
				.addClass('reverse')
				.css('bottom', gb.footer.outerHeight() + 'px');
			}else{
				gb.sideMenu
				.removeClass('reverse')
				.css('bottom', 'auto');
